...
const styles = StyleSheet.create({
  ...
  bandTop: { height: 12, backgroundColor: SKY, width: '100%' },
  lisereMid: { height: 3, backgroundColor: GOLD, width: '100%' },
  bandBottom: { height: 12, backgroundColor: SKY, width: '100%' },
  ...
  innerBorder: { position: 'absolute', top: 30, left: 30, right: 30, bottom: 30, borderWidth: 0.5, borderColor: SKY, borderStyle: 'solid' },
  ...
  brandLine: { fontSize: 11, fontFamily: 'Helvetica-Bold', color: SKY, letterSpacing: 3, marginBottom: 12, textAlign: 'center' },
  ruleIndigo: { width: 300, height: 0.75, backgroundColor: INDIGO_MID, marginBottom: 14 },
  pill: { backgroundColor: SKY, borderRadius: 20, paddingHorizontal: 16, paddingVertical: 5, marginBottom: 14 },
  ...
  certBox: { backgroundColor: SKY_BG, borderWidth: 0.75, borderColor: SKY, borderStyle: 'solid', borderRadius: 4, paddingHorizontal: 14, paddingVertical: 6, alignItems: 'center' },
  certNumber: { fontSize: 9.5, fontFamily: 'Helvetica-Bold', color: SKY, textAlign: 'center' },
...
export default function CourseCertificate({
  recipientName, formationTitle, subjectLabel, totalChapters, totalSubparts, certificateNumber, issuedAt, logoSrc,
}: Props) {
  const issuedDate = new Date(issuedAt).toLocaleDateString('fr-FR', {
    day: 'numeric', month: 'long', year: 'numeric',
  })
  const chapterLabel = totalChapters > 1 ? `${totalChapters} chapitres` : `${totalChapters} chapitre`
  const contentLabel = totalSubparts > 1 ? `contenus validés` : `contenu validé`
      ...
      <Page size="A4" orientation="landscape" style={styles.page}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.pill}>
            <Text style={styles.pillText}>E-LEARNING</Text>
          </View>
          <Text style={styles.titleMain}>Attestation de compl{'é'}tion</Text>
          <Text style={styles.subtitleFormation}>
            {(subjectLabel || 'FORMATION EN LIGNE').toUpperCase()}
          </Text>
          <View style={styles.ruleGold} />
          <Text style={styles.decerneLabel}>D{'É'}CERN{'É'} {'À'}</Text>
          <Text style={styles.recipientName}>{recipientName}</Text>
          <Text style={styles.praticienPour}>
            Pour avoir suivi et valid{'é'} l&apos;int{'é'}gralit{'é'} de la formation
          </Text>
          <Text style={styles.deckTitle}>{'«'} {formationTitle} {'»'}</Text>
          <Text style={styles.cardsLine}>
            {chapterLabel} {'·'} {totalSubparts} {contentLabel}
          </Text>
