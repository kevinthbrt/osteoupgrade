...
type ContentType = 'course' | 'case'

type Props = {
  contentType: ContentType
}

export default function FormationsLibrary({ contentType }: Props) {
  ...
  const isCaseLibrary = contentType === 'case'

  const copy = isCaseLibrary
    ? {
        heroBadge: 'Cas cliniques',
        heroTitle: 'Cas cliniques immersifs',
        heroDescription:
          'Analysez des situations réelles avec des vidéos, des photos et des quiz pour renforcer votre raisonnement clinique.',
        listTitle: 'Mes cas cliniques',
        listSubtitle: 'Sélectionnez un cas clinique pour progresser étape par étape.',
        searchPlaceholder: 'Rechercher un cas clinique',
        emptyTitle: 'Aucun cas clinique disponible',
        emptySubtitle: 'Les cas cliniques apparaîtront ici une fois ajoutés.',
        emptySearch: 'Aucun cas clinique ne correspond à votre recherche.',
        addButton: 'Ajouter un cas clinique',
        premiumLabel: 'Cas cliniques',
        premiumTitle: 'Cas cliniques interactifs',
        premiumDescription:
          'Accès réservé aux membres Premium. Scénarios cliniques avec vidéos, photos et quiz intégrés.',
        mediaLockedMessage: 'Terminez le quiz précédent pour débloquer ce contenu.'
      }
    : {
        heroBadge: 'Cours Premium',
        heroTitle: 'Formations vidéo',
        heroDescription:
          'Développez vos compétences avec nos parcours de formation structurés. Vidéos, exercices et suivi de progression inclus.',
        listTitle: 'Mes formations',
        listSubtitle: "Sélectionnez une formation pour commencer votre parcours d'apprentissage",
        searchPlaceholder: 'Rechercher une formation',
        emptyTitle: 'Aucune formation disponible',
        emptySubtitle: 'Les formations apparaîtront ici une fois ajoutées.',
        emptySearch: 'Aucune formation ne correspond à votre recherche.',
        addButton: 'Ajouter une formation',
        premiumLabel: 'Cours',
        premiumTitle: 'Formations en ligne',
        premiumDescription:
  ...
  const loadFormationsFromSupabase = async (userId: string, role?: string, preserveFormationId?: string) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          `id, title, description, is_private, photo_url, is_free_access, content_type,
          chapters:elearning_chapters(id, title, order_index,
            subparts:elearning_subparts(id, title, vimeo_url, image_url, description_html, order_index,
  ...
  const handleDeleteFormation = async (id: string) => {
    if (!confirm(`Êtes-vous sûr de vouloir supprimer ${isCaseLibrary ? 'ce cas clinique' : 'cette formation'} ? Cette action est irréversible.`)) {
      return
    }
    ...
    try {
      ...
      console.error('Impossible de supprimer le contenu', error)
      alert(`Erreur lors de la suppression ${isCaseLibrary ? 'du cas clinique' : 'de la formation'}`)
    }
  }
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm p-6">
              <div className="flex items-center gap-2 text-primary-600 text-sm font-semibold mb-3">
                {isCaseLibrary ? <Target className="h-5 w-5" /> : <GraduationCap className="h-5 w-5" />}
                {copy.premiumLabel}
              </div>
              <h1 className="text-2xl font-bold text-gray-900 mb-2">{copy.premiumTitle}</h1>
              <p className="text-gray-600">{copy.premiumDescription}</p>
            </div>
          ...
          <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl p-8 shadow-lg">
            <div className="max-w-2xl">
              ...
              <h2 className="text-3xl font-bold mb-4">
                Passez Premium pour débloquer les {isCaseLibrary ? 'cas cliniques' : 'cours'}
              </h2>
              <p className="text-white/90 text-lg mb-6">{copy.premiumDescription}</p>
              <button
    ...
    <AuthLayout>
      <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-start justify-between gap-6">
              <div className="max-w-4xl">
                <div className="inline-flex items-center gap-2 rounded-full bg-white/10 backdrop-blur-sm px-3 py-1.5 mb-4 border border-white/20">
                  ...
                  <span className="text-xs font-semibold text-blue-100">{copy.heroBadge}</span>
                </div>
        ...
        <div className="space-y-6">
            <div className="mb-4">
              ...
              <p className="text-slate-600">{copy.listSubtitle}</p>
            </div>
            ...
            {formations.length === 0 && (
              <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-gradient-to-br from-slate-50 to-white p-12 text-center">
                ...
                <p className="text-gray-600 text-sm">{copy.emptySubtitle}</p>
              </div>
            )}
                        ...
                        {expandedChapters[chapter.id] && (
                          <div className="divide-y divide-gray-100">
                            {chapter.subparts.map((subpart, subpartIdx) => {
                              ...
                              const hasVideo = Boolean(subpart.vimeo_url)
                              const hasImage = Boolean(subpart.image_url)
                              const mediaLabel = hasVideo ? 'Vidéo' : hasImage ? 'Photo' : 'Contenu'
                              const MediaIcon = hasVideo ? Video : hasImage ? Image : FileText
                                    ...
                                    <div className="flex-1 space-y-3">
                                      ...
                                      {!isAccessible && (
                                        <div className="flex items-center gap-2 text-sm text-orange-600 bg-orange-50 rounded-lg px-3 py-2 border border-orange-200">
                                          ...
                                          <span>{copy.mediaLockedMessage}</span>
                                        </div>
                                      )}
                                        ...
                                        <>
                                          ...
                                          <div className="space-y-3">
                                            {hasVideo ? (
                                              ...
                                              <div className="relative w-full overflow-hidden rounded-xl border-2 border-gray-200 bg-white shadow-lg">
                                              ...
                                              <div className="rounded-xl border-2 border-dashed border-gray-300 p-6 text-sm text-gray-500 bg-gray-50 text-center">
                                                <MediaIcon className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                                                {mediaLabel} à venir
                                              </div>
                                            )}
      ...
      {showWizard && (
        <CourseCreationWizard
          ...
          contentType={contentType}
          entityLabel={isCaseLibrary ? 'cas clinique' : 'formation'}
        />
      )}
