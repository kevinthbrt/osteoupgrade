-- Migration: Gamification System (Hybrid)
-- Description: Gamification basée sur user_sessions + completion e-learning
-- Author: Claude
-- Date: 2025-01-12
...
CREATE TABLE IF NOT EXISTS public.user_gamification_stats (
  ...
  -- Sessions (basées sur user_sessions)
  ...
  -- Taux de complétion (basé sur e-learning)
...
-- Description: Stats basées sur user_sessions + completion e-learning
-- ============================================================================
...
DECLARE
  ...
  -- E-learning variables pour completion
...
BEGIN
  ...
  -- ========================================================================
  -- PARTIE 1: Statistiques basées sur user_sessions (Consultation V3)
  -- ========================================================================
  SELECT
    COUNT(*),
  ...
  -- Calculer la série (streak) basée sur user_sessions
  ...
  -- ========================================================================
  -- PARTIE 2: Taux de complétion basé sur e-learning
  -- ========================================================================
  -- Compter le nombre total de subparts disponibles pour l'utilisateur
  -- et le nombre complété
  SELECT
  ...
  -- Calculer le taux de complétion e-learning
  IF v_total_subparts > 0 THEN
    v_completion_rate := ROUND((v_completed_subparts::NUMERIC / v_total_subparts::NUMERIC) * 100);
  ELSE
    v_completion_rate := 0;
  END IF;

  -- ========================================================================
  -- PARTIE 3: Insérer ou mettre à jour les stats
  -- ========================================================================
  INSERT INTO public.user_gamification_stats (
    ...
    v_week_sessions,
    v_completion_rate,  -- Basé sur e-learning !
    v_week_start,
    NOW()
  )
...
-- Trigger pour mettre à jour le taux de complétion e-learning
DROP TRIGGER IF EXISTS trigger_update_elearning_completion ON public.elearning_subpart_progress;
CREATE TRIGGER trigger_update_elearning_completion
...
INSERT INTO public.achievements (slug, name, description, icon, category, unlock_condition, points, gradient_from, gradient_to, display_order) VALUES
  ...
  -- Completion achievements (basé sur e-learning !)
  ('perfectionist', 'Perfectionniste', '80% de complétion e-learning', 'Star', 'completion', '{"type": "completion_rate", "value": 80}', 400, 'from-emerald-400', 'to-green-500', 20),
  ('flawless', 'Impeccable', '95% de complétion e-learning', 'CheckCircle2', 'completion', '{"type": "completion_rate", "value": 95}', 800, 'from-green-400', 'to-emerald-500', 21),
...
COMMENT ON TABLE public.user_gamification_stats IS 'Statistiques de gamification : sessions/tests/streaks basés sur user_sessions, complétion basée sur e-learning';
COMMENT ON FUNCTION public.update_user_gamification_stats IS 'Met à jour les stats : user_sessions pour gamification principale, e-learning pour taux de complétion';
