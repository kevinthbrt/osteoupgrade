...
CREATE OR REPLACE FUNCTION pg_temp.bloc_code() RETURNS text AS $fn$
  SELECT '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-bottom: 22px;"><tr><td style="padding: 18px; background-color: #f5f3ff; border: 1px solid #ddd6fe; border-radius: 8px; text-align: center;">'
      ...
      -- Le code est rattaché à l'adresse laissée sur la page. Créer son
      -- compte avec une autre reviendrait à payer plein tarif sans comprendre
      -- pourquoi.
      || '<p style="margin: 6px 0 0; font-size: 13px; color: #6b7280;">Pensez à créer votre compte avec cette même adresse email.</p>'
      || '</td></tr></table>'
$fn$ LANGUAGE sql;
